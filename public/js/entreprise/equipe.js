 ...
 */
let tab_Personne = [
    ["Docteur en chirurgie dentaire", "Ingénieur Ecole Centrale Marseille", "Master CAAE IAE Montpellier", "Fondateur de Sup'Perform", "18 ans d'expérience dans l'enseignement supérieur",],
    ["Titulaire d’un Master II en droit de l’Université de Montpellier en 2015", "Titulaire du D.U Certificat d’Études Judiciaires", "Juriste formatrice au profit de différentes CCI et organismes de formation"],
    [ "Docteur en droit de l’Université de Montpellier  en 2015", "Enseignant en Licence et Master \n à la Faculté de droit de Montpellier (2010-2017)", "Juriste formateur pour les collectivités territoriales", "10 ans d'expérience dans l’enseignement"]];

function eventModal(){

    anthony.addEventListener("mouseover", function (){
        creer_info_personne(0);
        afficher_equipe_top("prenom_anthony");
        afficher_equipe_top("info_equipe");
    })

    anthony.addEventListener("mouseout", function (){
        cacher_equipe_top("prenom_anthony");
        cacher_equipe_top("info_equipe");
    })

    lea.addEventListener("mouseover", function (){
        creer_info_personne(1);
        afficher_equipe_top("prenom_lea");
        afficher_equipe_top("info_equipe");
    })

    lea.addEventListener("mouseout", function (){
        cacher_equipe_top("prenom_lea");
        cacher_equipe_top("info_equipe");
    })

    jonathan.addEventListener("mouseover", function (){
        creer_info_personne(2);
        afficher_equipe_top("prenom_jonathan");
        afficher_equipe_top("info_equipe");
    })

    jonathan.addEventListener("mouseout", function (){
        cacher_equipe_top("prenom_jonathan");
        cacher_equipe_top("info_equipe");
    })
...
function creer_info_personne(numeroPersonne){
    ...
    for (let i = 0; i < tab_Personne[numeroPersonne].length; i++) {
